...
import * as dataFormat from 'pages/CRUD/Records/table/RecordsDataFormatters';

// eslint-disable-next-line
import * as usersDataFormat from 'pages/CRUD/Users/table/UsersDataFormatters';
// eslint-disable-next-line
import * as projectsDataFormat from 'pages/CRUD/Projects/table/ProjectsDataFormatters';

import actions from 'actions/records/recordsListActions';
...
const RecordsTable = ({ classes }) => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Hours', title: 'hours', number: 'true' },
  ...
  const count = useSelector((store) => store.records.list.count);
  const modalOpen = useSelector((store) => store.records.list.modalOpen);
  const rows = useSelector((store) => store.records.list.rows);
  const idToDelete = useSelector((store) => store.records.list.idToDelete);
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) =>
        projectsDataFormat.listFormatter(
          params.row[params.field],
          history,
          'projects',
        ),
      ...
      <div>
        ...
        >
          <MenuItem
            classes={classes}
            onClick={() => {
              history.push(`/admin/records/${cell.id}/edit`);
              handleClose();
            }}
          >
            <EditIcon />
  ...
  return (
    <div>
      <Widget title={<h4>Records</h4>} disableWidgetMenu>
          ...
          <a
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Records'
                : 'http://localhost:8080/api-docs/#/Records'
            }
          >
            API documentation for records
          </a>
        </div>
      </Widget>
