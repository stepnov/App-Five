...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/projects/autocomplete`, { params });
  return response.data;
}

const ProjectsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.projects.hasPermissionToCreate,
});

export default connect(select)(ProjectsSelectItem);
