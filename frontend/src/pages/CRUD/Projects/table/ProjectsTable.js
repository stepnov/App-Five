...
import * as dataFormat from 'pages/CRUD/Projects/table/ProjectsDataFormatters';

import actions from 'actions/projects/projectsListActions';
...
const ProjectsTable = ({ classes }) => {
  ...
  const [filters, setFilters] = React.useState([
    ...
    { label: 'Rate', title: 'rate', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.projects.list.count);
  const modalOpen = useSelector((store) => store.projects.list.modalOpen);
  const rows = useSelector((store) => store.projects.list.rows);
  const idToDelete = useSelector((store) => store.projects.list.idToDelete);
      ...
      <div>
        ...
        >
          <MenuItem
            classes={classes}
            onClick={() => {
              history.push(`/admin/projects/${cell.id}/edit`);
              handleClose();
            }}
          >
            <EditIcon />
  ...
  return (
    <div>
      <Widget title={<h4>Projects</h4>} disableWidgetMenu>
          ...
          <a
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Projects'
                : 'http://localhost:8080/api-docs/#/Projects'
            }
          >
